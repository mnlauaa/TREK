import {
  PLUGIN_SESSION_MAX_KEYS,
  PLUGIN_SESSION_MAX_KEY_LENGTH,
  PLUGIN_SESSION_MAX_VALUE_BYTES,
} from "./index.js";
import type {
  PluginContext,
  PluginDefinition,
...
export interface MockHostOptions {
  ...
  declaredActions?: Array<
    string | { key: string; scope?: "user" | "instance" }
  >;
...
export interface PluginDriver {
  ...
  route(
    match: number | { method: string; path: string },
...
export interface MockHost {
  ...
  scheduled: Map<
    string,
    { dueAt: number; everyMs?: number; payload?: unknown }
  >;
...
const FORBIDDEN_SQL =
  /\b(ATTACH|DETACH|VACUUM|PRAGMA|RECURSIVE|LOAD_EXTENSION)\b/i;
...
function guardSql(sql: string): void {
  ...
  if (FORBIDDEN_SQL.test(sql))
    throw new Error("statement type not allowed for plugin databases");
}
...
const PLACE_STR_LIMITS: Record<string, number> = {
  name: 200,
  description: 2000,
  address: 500,
  notes: 2000,
};
const TRIP_STR_LIMITS: Record<string, number> = {
  title: 200,
  description: 2000,
};
...
export function createMockHost(opts: MockHostOptions = {}): MockHost {
  ...
  const sessionPrefix = (scope: "plugin" | "trip") => {
    if (scope === "trip" && opts.sessionTripId === undefined) {
      throw sessionError(
        "NO_TRIP_CONTEXT",
        "trip session storage requires a trip context",
      );
    }
  ...
  const sessionKey = (key: string, scope: "plugin" | "trip") => {
    ...
    if (!key || key.length > PLUGIN_SESSION_MAX_KEY_LENGTH) {
      throw sessionError(
        "SESSION_INVALID_KEY",
        `session key must be 1-${PLUGIN_SESSION_MAX_KEY_LENGTH} characters`,
      );
    }
    return `${prefix}${key}`;
  };
  const session: PluginSessionStorage = {
    async get(key, options) {
      const raw = sessionValues.get(
        sessionKey(key, options?.scope === "trip" ? "trip" : "plugin"),
      );
    ...
    async set(key, value, options) {
      const fullKey = sessionKey(
        key,
        options?.scope === "trip" ? "trip" : "plugin",
      );
      const serialized = JSON.stringify(value);
      if (serialized === undefined)
        throw sessionError(
          "SESSION_INVALID_VALUE",
          "session value must be JSON-serialisable",
        );
      const valueBytes = Buffer.byteLength(serialized, "utf8");
      if (valueBytes > PLUGIN_SESSION_MAX_VALUE_BYTES) {
        throw sessionError(
          "SESSION_VALUE_TOO_LARGE",
          `session value exceeds ${PLUGIN_SESSION_MAX_VALUE_BYTES} bytes`,
        );
      }
      const prefix = sessionPrefix(
        options?.scope === "trip" ? "trip" : "plugin",
      );
      const scopedKeyCount = [...sessionValues.keys()].filter((storedKey) =>
        storedKey.startsWith(prefix),
      ).length;
      if (
        !sessionValues.has(fullKey) &&
        scopedKeyCount >= PLUGIN_SESSION_MAX_KEYS
      ) {
        throw sessionError(
          "SESSION_KEY_LIMIT",
          `plugin session storage allows at most ${PLUGIN_SESSION_MAX_KEYS} keys`,
        );
      }
      sessionValues.set(fullKey, serialized);
    },
    async remove(key, options) {
      sessionValues.delete(
        sessionKey(key, options?.scope === "trip" ? "trip" : "plugin"),
      );
    },
    async clear(options) {
  ...
  const need = (perm: string, method: string) => {
    ...
    if (!grants.has(perm))
      throw new PermissionDenied(
        `PERMISSION_DENIED: ${method} requires ${perm}`,
      );
  };
  const assertMember = (tripId: number, asUserId: number) => {
    const t = opts.trips?.[tripId];
    if (!t || !t.members.includes(asUserId))
      throw new Error(`RESOURCE_FORBIDDEN: no access to trip ${tripId}`);
    return t;
  };
  ...
  const requireAddon = (enabled: boolean | undefined, noun: string) => {
    if (enabled === false)
      throw new Error(`RESOURCE_FORBIDDEN: the ${noun} addon is disabled`);
  };
  const assertEdit = (
    t: {
      canEditPlaces?: boolean;
    ...
    if (t[flag] === false)
      throw new Error(
        `RESOURCE_FORBIDDEN: no permission to edit trip ${tripId}`,
      );
  };
  ...
  const assertRight = (
    t: { can?: Record<string, boolean> },
    right: string,
    tripId: number,
  ) => {
    if (t.can?.[right] === false)
      throw new Error(
        `RESOURCE_FORBIDDEN: no permission to edit trip ${tripId}`,
      );
  };
  ...
  const shortCode = (v: unknown, name: string): string => {
    if (typeof v !== "string" || v.trim() === "" || v.length > 8)
      throw new Error(`${name} must be a short code`);
  ...
  const rows = (arr: unknown[] | undefined): Array<Record<string, unknown>> =>
  ...
  const sharesTrip = (a: number, b: number) =>
    Object.values(opts.trips ?? {}).some(
      (t) => t.members.includes(a) && t.members.includes(b),
    );
  ...
  const scheduleTask = (
    ...
    if (!name || name.length > 128)
      throw new Error(`scheduler name is required (max 128 chars)`);
    if (
      !Number.isFinite(dueAt) ||
      dueAt > Date.now() + 366 * 24 * 60 * 60 * 1000
    )
      throw new Error("scheduler dueAt out of range");
    if (
      everyMs !== undefined &&
      (!Number.isFinite(everyMs) || everyMs < 60_000)
    )
      throw new Error("recurring interval must be >= 60000 ms");
    if (JSON.stringify(payload ?? null).length > 8 * 1024)
      throw new Error("scheduler payload too large (max 8192 bytes)");
    if (!scheduledTasks.has(name) && scheduledTasks.size >= 100)
      throw new Error("too many scheduled tasks (max 100)");
  ...
  const metaKey = (et: string, eid: number, key: string) =>
    `${et}:${eid}:${key}`;
  type MockTrip = NonNullable<MockHostOptions["trips"]>[number];
  let exchangeRateSequence = 0;
  const positiveRate = (value: unknown): value is number =>
    typeof value === "number" && Number.isFinite(value) && value > 0;
  const tripCurrency = (trip: MockTrip): string =>
    String(
      (trip.data as { currency?: unknown } | undefined)?.currency ?? "EUR",
    ).toUpperCase();
  const resolveFixtureRate = (
    ...
    const currency = String(input || base).toUpperCase();
    if (currency === base) {
      return {
        trip_id: tripId,
        trip_currency: base,
        ...
        exchange_rate: 1,
        source: "identity",
        source_version: `identity:${base}`,
        effective_date: null,
        fetched_at: null,
        stale: false,
      };
    }
    const tripRate = (trip.rates ?? []).find(
    ...
    if (tripRate) {
      return {
        ...
        effective_date:
          typeof tripRate.effective_date === "string"
            ? tripRate.effective_date
            : null,
    ...
    return {
      ...
      source: "global",
      source_version: `mock-global:${base}`,
      effective_date: null,
  ...
  const freezeFixtureRate = (
    ...
    const currency = String(
      input.currency ?? existing?.currency ?? base,
    ).toUpperCase();
    const oldCurrency = existing
      ? String(existing.currency ?? base).toUpperCase()
      : null;
    let rate: number;
    ...
    } else if (input.exchange_rate !== undefined) {
      if (!positiveRate(input.exchange_rate))
        throw new Error("exchange_rate must be finite and greater than zero");
      rate = input.exchange_rate;
      source = "explicit";
      sourceVersion = `explicit:mock-${++exchangeRateSequence}`;
    } else {
      if (existing && currency === oldCurrency) return;
      const resolution = resolveFixtureRate(tripId, trip, currency);
      if (!resolution)
        throw new Error("no exchange rate is available; enter a manual rate");
      rate = resolution.exchange_rate;
      source = resolution.source;
      sourceVersion = resolution.source_version;
  ...
  const buildCtx = (actingUserId: number | undefined): PluginContext => {
    const requireActingUser = (): number => {
      if (actingUserId === undefined)
        throw new Error(
          "RESOURCE_FORBIDDEN: this call requires an authenticated user context",
        );
      return actingUserId;
    };
    ...
    return {
      ...
      db: {
        ...
        async tx(ops) {
          ...
          if (ops.length > MAX_TX_OPS)
            throw new Error(`tx allows at most ${MAX_TX_OPS} statements`);
          for (const op of ops) {
            guardSql(op.sql);
            if (TX_CONTROL.test(stripLeadingComments(op.sql)))
              throw new Error(
                "transaction-control statements are not allowed inside tx()",
              );
          }
      ...
      trips: {
        async getById(tripId, _asUserId) {
          ...
          return (assertMember(tripId, requireActingUser()).data ??
            null) as Trip | null;
        },
        async getPlaces(tripId, _asUserId) {
          need("db:read:trips", "trips.getPlaces");
          return (assertMember(tripId, requireActingUser()).places ??
            []) as Place[];
        },
        async getReservations(tripId, _asUserId) {
          need("db:read:trips", "trips.getReservations");
          return (assertMember(tripId, requireActingUser()).reservations ??
            []) as Reservation[];
        },
        async getDays(tripId) {
          need("db:read:trips", "trips.getDays");
          return (assertMember(tripId, requireActingUser()).days ??
            []) as Day[];
        },
        ...
        async create(input) {
          ...
          if (typeof input.title !== "string" || input.title === "")
            throw new Error("invalid trip: title is required");
          capStrings(input as Record<string, unknown>, TRIP_STR_LIMITS);
          const id =
            Math.max(0, ...Object.keys(opts.trips ?? {}).map(Number)) + 1;
        ...
        async members(tripId) {
          ...
          return assertMember(tripId, requireActingUser()).members.map(
            (id) => opts.users?.[id] ?? { id },
          ) as User[];
        },
        async addMember(tripId, userId) {
          ...
          if (!opts.users?.[userId])
            throw new Error(`RESOURCE_FORBIDDEN: no user ${userId}`);
        ...
        async removeMember(tripId, userId) {
          ...
          if (
            (t.data as { user_id?: number } | undefined)?.user_id === userId
          ) {
      ...
      reservations: {
        ...
        async update(tripId, reservationId, input) {
          ...
          if (!r)
            throw new Error(
              `RESOURCE_FORBIDDEN: no reservation ${reservationId} on trip ${tripId}`,
            );
      ...
      accommodations: {
        async create(tripId, input) {
          ...
          if (
            !Math.trunc(Number(input.place_id)) ||
            !Math.trunc(Number(input.start_day_id)) ||
            !Math.trunc(Number(input.end_day_id))
          ) {
            throw new Error(
              "place_id, start_day_id, and end_day_id are required",
            );
          }
          assertEdit(t, "canEditDays", tripId);
          const a = {
            id: (t.accommodations?.length ?? 0) + 1,
            trip_id: tripId,
            ...input,
          };
        ...
        async update(tripId, accommodationId, input) {
          ...
          const t = assertMember(tripId, requireActingUser());
          assertEdit(t, "canEditDays", tripId);
          const a = rows(t.accommodations).find(
            (x) => x.id === accommodationId,
          );
          if (!a)
            throw new Error(
              `RESOURCE_FORBIDDEN: no accommodation ${accommodationId} on trip ${tripId}`,
            );
      ...
      packing: {
        ...
        async create(tripId, input) {
          ...
          if (typeof input.name !== "string" || input.name === "")
            throw new Error("invalid packing item: name is required");
          const t = assertMember(tripId, uid);
          assertRight(t, "packing_edit", tripId);
          const item = {
            id: (t.packing?.length ?? 0) + 1,
            trip_id: tripId,
            ...input,
          };
          (t.packing ??= []).push(item);
        ...
        async update(tripId, itemId, input) {
          ...
          if (!item)
            throw new Error(
              `RESOURCE_FORBIDDEN: no packing item ${itemId} on trip ${tripId}`,
            );
        ...
        async updateBag(tripId, bagId, input) {
          ...
          if (!bag)
            throw new Error(
              `RESOURCE_FORBIDDEN: no bag ${bagId} on trip ${tripId}`,
            );
      ...
      files: {
        ...
        async getContent(tripId, fileId) {
          ...
          if (!file)
            throw new Error(
              `RESOURCE_FORBIDDEN: no file ${fileId} on trip ${tripId}`,
            );
          const content =
            typeof file.content_base64 === "string" ? file.content_base64 : "";
          return {
        ...
        async create(tripId, input) {
          ...
          if (
            ...
            throw new Error("file name is required (max 255 chars)");
          if (typeof content_base64 !== "string" || content_base64 === "")
            throw new Error("content_base64 is required");
          if (content_base64.length > 14 * 1024 * 1024)
            throw new Error("file exceeds the 10MB plugin upload cap");
          const t = assertMember(tripId, uid);
          assertRight(t, "file_upload", tripId);
          const file = {
            id: (t.files?.length ?? 0) + 1,
            trip_id: tripId,
      ...
      collab: {
        ...
        async listMessages(tripId, before) {
          ...
          return before === undefined
            ? messages
            : messages.filter((m) => typeof m.id === "number" && m.id < before);
        },
        ...
        async createMessage(tripId, text, replyTo) {
          ...
          if (
            ...
            throw new Error("message text is required (max 4000 chars)");
          const t = assertMember(tripId, uid);
          assertRight(t, "collab_edit", tripId);
          requireAddon(opts.collabAddonEnabled, "collab");
          return {
      ...
      notify: {
        async send(input) {
          ...
          const title =
            typeof input.title === "string" ? stripEmoji(input.title) : "";
          const body =
            typeof input.body === "string" ? stripEmoji(input.body) : "";
          if (!title || title.length > 200)
            throw new Error("notification title is required (max 200 chars)");
          if (!body || body.length > 1000)
            throw new Error("notification body is required (max 1000 chars)");
          const scope: unknown = input.scope;
          if (scope !== "user" && scope !== "trip")
            throw new Error("scope must be 'user' or 'trip'");
      ...
      ai: {
        async complete() {
          ...
          if (aiUsed >= aiPerDay)
            throw new Error(
              "daily AI budget exhausted (resets at UTC midnight)",
            );
          aiUsed += 1;
      ...
      tags: {
        ...
        async update(tagId, input) {
          ...
          if (!tag)
            throw new Error(
              `RESOURCE_FORBIDDEN: no tag ${tagId} for this user`,
            );
      ...
      todos: {
        ...
        async update(tripId, todoId, input) {
          ...
          if (!todo)
            throw new Error(
              `RESOURCE_FORBIDDEN: no todo ${todoId} on trip ${tripId}`,
            );
      ...
      journal: {
        ...
        async getEntries(journeyId) {
          ...
          requireActingUser();
          requireAddon(opts.journeyAddonEnabled, "journey");
          if (!rows(journals).some((x) => x.id === journeyId))
            throw new Error(
              `RESOURCE_FORBIDDEN: no access to journey ${journeyId}`,
            );
        ...
        async createEntry(journeyId, input) {
          ...
          if (!rows(journals).some((x) => x.id === journeyId))
            throw new Error(
              `RESOURCE_FORBIDDEN: no editable journey ${journeyId} for this user`,
            );
          const entry = {
        ...
        async addEntryPhoto(entryId, input) {
          ...
          if (!entry)
            throw new Error(
              `RESOURCE_FORBIDDEN: no editable journal entry ${entryId} for this user`,
            );
          if (typeof input?.name !== "string" || input.name.trim() === "")
            throw new Error("invalid photo input: name is required");
          if (
            typeof input?.content_base64 !== "string" ||
            input.content_base64 === ""
          )
            throw new Error("invalid photo input: content_base64 is required");
          ...
          if (
            ...
            throw new Error(
              `photo extension '${ext || "(none)"}' is not an allowed image type`,
            );
          }
          if (input.content_base64.length > 14 * 1024 * 1024)
            throw new Error("photo exceeds the 10MB plugin upload cap");
          const photo = {
            id: journalPhotoSeq++,
            photo_id: journalPhotoSeq,
            entry_id: entryId,
        ...
        async deleteJourney(journeyId) {
          ...
          if (i < 0)
            throw new Error(
              `RESOURCE_FORBIDDEN: no deletable journal ${journeyId} for this user`,
            );
          journals.splice(i, 1);
          return { deleted: true };
        },
      ...
      atlas: {
        ...
        async markRegion(regionCode, countryCode, regionName) {
          ...
          const name =
            typeof regionName === "string" && regionName
              ? regionName.slice(0, 128)
              : String(regionCode ?? "");
          const region = {
            region_code: shortCode(regionCode, "regionCode"),
            country_code: shortCode(countryCode, "countryCode"),
            region_name: name,
          };
          requireAddon(opts.atlasAddonEnabled, "atlas");
        ...
        async createBucketItem(input) {
          ...
          requireActingUser();
          if (typeof input.name !== "string" || input.name.trim() === "")
            throw new Error("bucket item name is required");
          requireAddon(opts.atlasAddonEnabled, "atlas");
        ...
        async deleteBucketItem(itemId) {
          ...
          if (i < 0)
            throw new Error(
              `RESOURCE_FORBIDDEN: no bucket item ${itemId} for this user`,
            );
      ...
      vacay: {
        ...
        async toggleEntry(date) {
          ...
          requireAddon(opts.vacayAddonEnabled, "vacay");
          if (vacayEntries.has(d)) {
            vacayEntries.delete(d);
        ...
        async toggleCompanyHoliday(date) {
          ...
          const d = vacayDate(date);
          requireAddon(opts.vacayAddonEnabled, "vacay");
          if (vacayHolidays.has(d)) {
            vacayHolidays.delete(d);
            return { action: "removed" };
          }
          vacayHolidays.add(d);
          return { action: "added" };
      ...
      collections: {
        ...
        async create(input) {
          ...
          if (
            ...
            throw new Error(
              "invalid collection: name is required (max 120 chars)",
            );
          requireAddon(opts.collectionsAddonEnabled, "collections");
        ...
        async savePlace(input) {
          ...
          requireActingUser();
          if (typeof input.collection_id !== "number")
            throw new Error("invalid place: collection_id is required");
          if (typeof input.name !== "string" || input.name === "")
            throw new Error("invalid place: name is required");
          requireAddon(opts.collectionsAddonEnabled, "collections");
        ...
        async copyToTrip(input) {
          ...
          requireActingUser();
          if (typeof input.trip_id !== "number")
            throw new Error("invalid copy request: trip_id is required");
          if (!Array.isArray(input.place_ids) || input.place_ids.length < 1)
            throw new Error(
              "invalid copy request: place_ids must be a non-empty array",
            );
          requireAddon(opts.collectionsAddonEnabled, "collections");
        ...
        async deletePlace(placeId) {
          ...
          if (i < 0)
            throw new Error(`RESOURCE_FORBIDDEN: no saved place ${placeId}`);
      ...
      daynotes: {
        ...
        async update(tripId, dayId, noteId, input) {
          ...
          const t = assertMember(tripId, requireActingUser());
          assertEdit(t, "canEditDays", tripId);
          const note = rows(t.daynotes).find(
            (x) => x.id === noteId && x.day_id === dayId,
          );
          if (!note)
            throw new Error(
              `RESOURCE_FORBIDDEN: no note ${noteId} on day ${dayId}`,
            );
      ...
      costs: {
        ...
        async create(tripId, input) {
          ...
          requireAddon(opts.budgetAddonEnabled, "costs");
          if (typeof input.name !== "string" || input.name === "")
            throw new Error("invalid cost: name is required");
          const t = assertMember(tripId, uid);
          if (t.canEditCosts === false) {
            throw new Error(
              `RESOURCE_FORBIDDEN: no permission to edit costs on trip ${tripId}`,
            );
          }
          const frozen = { ...input };
          freezeFixtureRate(tripId, t, frozen, uid);
          const item = {
            id: (t.costs?.length ?? 0) + 1,
            trip_id: tripId,
            ...frozen,
          };
          (t.costs ??= []).push(item);
        ...
        async update(tripId, itemId, input) {
          ...
          if (!item)
            throw new Error(
              `RESOURCE_FORBIDDEN: no cost ${itemId} on trip ${tripId}`,
            );
          const frozen = { ...input };
        ...
        async setRate(tripId, currencyInput, exchangeRate, note) {
          ...
          if (!positiveRate(exchangeRate))
            throw new Error(
              "exchange_rate must be finite and greater than zero",
            );
          const currency = currencyInput.toUpperCase();
          if (currency === tripCurrency(t))
            throw new Error(
              "the trip currency always uses a 1:1 identity rate",
            );
          ...
          const i = rates.findIndex(
            (candidate) => candidate.currency.toUpperCase() === currency,
          );
          if (i < 0) rates.push(rate);
          else rates[i] = rate;
          return rate;
        },
        async deleteRate(tripId, currencyInput) {
          ...
          const i = rates.findIndex(
            (candidate) =>
              candidate.currency.toUpperCase() === currencyInput.toUpperCase(),
          );
          if (i < 0)
            throw new Error(
              `RESOURCE_FORBIDDEN: no trip exchange rate for ${currencyInput}`,
            );
          rates.splice(i, 1);
          return { deleted: true };
        },
        ...
        async updateSettlement(tripId, settlementId, input) {
          ...
          const settlement = (t.settlements ?? []).find(
            (candidate) => candidate.id === settlementId,
          );
          if (!settlement)
            throw new Error(
              `RESOURCE_FORBIDDEN: no settlement ${settlementId} on trip ${tripId}`,
            );
          const frozen = { ...input };
      ...
      places: {
        ...
        async update(tripId, placeId, input) {
          ...
          if (!place)
            throw new Error(
              `RESOURCE_FORBIDDEN: no place ${placeId} on trip ${tripId}`,
            );
      ...
      days: {
        ...
        async update(tripId, dayId, input) {
          ...
          if (!day)
            throw new Error(
              `RESOURCE_FORBIDDEN: no day ${dayId} on trip ${tripId}`,
            );
      ...
      itinerary: {
        ...
        async unassign(tripId, assignmentId) {
          ...
          if (i < 0)
            throw new Error(
              `RESOURCE_FORBIDDEN: no assignment ${assignmentId} on trip ${tripId}`,
            );
          list.splice(i, 1);
          return { deleted: true };
        },
      ...
      meta: {
        ...
        async set(entityType, entityId, key, value) {
          ...
          if (key.length > META_KEY_MAX)
            throw new Error(`metadata key too long (>${META_KEY_MAX} chars)`);
          ...
          if (json.length > META_VALUE_MAX) {
            throw new Error(
              `metadata value too large (>${META_VALUE_MAX} bytes)`,
            );
          }
          const fullKey = metaKey(entityType, entityId, key);
          if (!(fullKey in metaStore)) {
            const prefix = `${entityType}:${entityId}:`;
            const count = Object.keys(metaStore).filter((k) =>
              k.startsWith(prefix),
            ).length;
            if (count >= META_KEYS_MAX)
              throw new Error(
                `too many metadata keys on this ${entityType} (max ${META_KEYS_MAX})`,
              );
          }
        ...
        async list(entityType, entityId) {
          ...
          for (const k of Object.keys(metaStore))
            if (k.startsWith(prefix))
              out[k.slice(prefix.length)] = metaStore[k];
          return out;
        },
      ...
      users: {
        async getById(id) {
          ...
          const uid = requireActingUser();
          if (id !== uid && !sharesTrip(uid, id))
            throw new Error(`RESOURCE_FORBIDDEN: no access to user ${id}`);
      ...
      ws: {
        async broadcastToTrip(tripId, event, data) {
          ...
          if (actingUserId === undefined)
            throw new Error(
              "RESOURCE_FORBIDDEN: broadcasts require an authenticated user context",
            );
        ...
        async broadcastToUser(userId, event, data) {
          ...
          if (actingUserId === undefined || userId !== actingUserId) {
            throw new Error(
              "RESOURCE_FORBIDDEN: a plugin may only broadcast to the acting user",
            );
          }
      ...
      plugins: {
        async call(pluginId, fn, args) {
          ...
          if (typeof impl !== "function")
            throw new Error(
              `RESOURCE_FORBIDDEN: plugin ${pluginId} does not export "${fn}"`,
            );
          return impl(args);
        },
      },
      events: {
        emit(name, payload) {
          ...
          if (opts.declaredEmits && !opts.declaredEmits.includes(name)) {
            console.warn(
              `[mock-host] events.emit("${name}") is not declared in capabilities.emits — production drops this event`,
            );
            return;
          }
  ...
  const run = (def: PluginDefinition): PluginDriver => ({
    ...
    route: async (match, req) => {
      ...
      const r =
        typeof match === "number"
          ? routes[match]
          : routes.find(
              (x) => x.method === match.method && x.path === match.path,
            );
      if (!r)
        throw new Error(
          `no route ${typeof match === "number" ? `#${match}` : `${match.method} ${match.path}`}`,
        );
      const full: PluginRequest = {
        ...
        user:
          opts.actingUserId != null
            ? { id: opts.actingUserId, username: "mock", isAdmin: false }
            : null,
        ...(req as object),
      };
    ...
    scheduled: async (name, payload) => {
      ...
      if (typeof def.scheduled !== "function")
        throw new Error("plugin has no scheduled handler");
    ...
    pluginEvent: async (plugin, event, payload) => {
      for (const s of def.subscriptions ?? []) {
        if (s.plugin === plugin && s.event === event)
          await s.handler(payload, userlessCtx);
      }
    },
    deleteUserData: async (userId) => {
      needEntry(USER_DATA_PERMISSION, "deleteUserData");
      if (typeof def.deleteUserData !== "function")
        throw new Error("plugin has no deleteUserData handler");
      await def.deleteUserData({ userId }, userlessCtx);
    },
    exportUserData: async (userId) => {
      needEntry(USER_DATA_PERMISSION, "exportUserData");
      if (typeof def.exportUserData !== "function")
        throw new Error("plugin has no exportUserData handler");
    ...
    hook: async (name, fn, ...args) => {
      ...
      if (!impl || typeof impl[fn] !== "function")
        throw new Error(`no hook ${name}.${fn}`);
      ...
      return impl[fn](
        ...args,
        name === "notificationChannel" ? userlessCtx : ctx,
      ) as never;
    },
    action: async (key) => {
      const declared = opts.declaredActions?.map((a) =>
        typeof a === "string" ? a : a.key,
      );
      if (declared && !declared.includes(key)) {
        throw new Error(
          `RESOURCE_FORBIDDEN: plugin did not declare action "${key}"`,
        );
      }
      ...
      try {
        ...
        return {
          ok: false,
          message: cap(e instanceof Error ? e.message : "Action failed"),
        };
      }
      return {
        ok: raw?.ok !== false,
        message: raw?.message === undefined ? undefined : cap(raw.message),
      };
    },
    channel: {
      send: async (msg, config) => {
        needEntry(
          HOOK_PERMISSION.notificationChannel,
        ...
        if (!allowed.includes(msg.event)) {
          throw new Error(
            `the host never dispatches "${msg.event}" to a plugin channel`,
          );
        }
        await impl.send(
          msg,
          settingsBlob(config ?? opts.userSettings, "channel config"),
          userlessCtx,
        );
      },
      test: async (config) => {
        ...
        const impl = def.hooks?.notificationChannel;
        if (typeof impl?.test !== "function")
          throw new Error("plugin has no notificationChannel.test hook");
        await impl.test(
          settingsBlob(config ?? opts.userSettings, "channel config"),
          userlessCtx,
        );
