...
describe('MCostSheet', () => {
  beforeEach(() => {
    ...
    useTripStore.setState({ addBudgetItem, updateBudgetItem, deleteBudgetItem } as unknown as Partial<TripStoreState>);
    addToast = vi.fn();
  ...
  it('FE-MOB-COSTSH-021: a note typed on the phone is saved with the expense (#1658)', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/what it covered/i), {
      target: { value: 'Ben was ill, he pays this back' },
    });
  ...
  it('FE-MOB-COSTSH-022: an existing note is loaded for editing, and a receipt blob never is (#1658)', () => {
    const { unmount } = renderSheet({
      editing: buildBudgetItem({ id: 5, name: 'Chemist', note: 'reimburse from the kitty' }),
    });
  ...
  it('FE-MOB-COSTSH-006: switching the category updates the pressed pill and the payload', async () => {
    ...
    fireEvent.click(submit());
    await waitFor(() =>
      expect(addBudgetItem).toHaveBeenCalledWith(1, expect.objectContaining({ category: 'transport' }))
    );
  });
  ...
  it('FE-MOB-COSTSH-007: a foreign currency shows inherited Trip-accounting conversion', async () => {
    ...
    expect(screen.getByRole('dialog').textContent).toContain('$100.00');
  ...
  it('FE-MOB-COSTSH-026: a failing delete disarms the button and reports the error', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('button', { name: 'Delete' }).className).not.toContain('--m-st-danger')
    );
  });
