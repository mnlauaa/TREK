...
describe('MCostsTab', () => {
  ...
  it('FE-MOB-COSTT-021: filters by a category that is actually in use', async () => {
    ...
    expect(options).toEqual([
      'costs.filter.allCategories',
      'costs.cat.food',
      'costs.cat.transport',
      'costs.cat.activities',
      'costs.cat.shopping',
    ]);
  ...
  it('FE-MOB-COSTT-022: filters by expense day, labelling known days with their number', async () => {
    ...
    expect(
      within(menu)
        .getAllByRole('button')
        .map((b) => b.textContent)
    ).toEqual(['costs.filter.allDays', 'Apr 30', 'dayplan.dayN:2 · May 2']);
  ...
  it('FE-MOB-COSTT-036: falls back to EUR and a neutral avatar without a user or trip currency', async () => {
    ...
    render(
      <MCostsTab
        planner={planner({ trip: null as unknown as TripPlanner['trip'], tripMembers: solo })}
        shell={buildShell()}
      />
    );
    await screen.findByText('+12,50 €');
  ...
  it.each([
    ...
    'keeps frozen FX %s and edit/delete actions on the single day-grouped payment row',
    async (rate, displayed) => {
      const payment = {
        id: 503,
        trip_id: 7,
        from_user_id: 1,
      ...
      const remove = vi.spyOn(budgetApi, 'deleteSettlement').mockResolvedValue({});
      serveSettlement({ ...SETTLEMENT, settlements: [payment] });
      await renderTab();
      expect(screen.getAllByText('costs.payment')).toHaveLength(1);
      const row = rowOf('costs.payment');
      // Rates store GBP per USD: the frozen 0.8 yields 25 / 0.8 = 31.25,
      // rather than the current quote's 25 / 0.5 = 50.
      expect(within(row).getByText(displayed)).toBeInTheDocument();
      ...
      await waitFor(() =>
        expect(update).toHaveBeenCalledWith(7, 503, {
          from_user_id: 1,
          to_user_id: 2,
          amount: 30,
          currency: 'GBP',
      ...
      fireEvent.click(
        within(screen.getByRole('dialog', { name: 'common.delete' })).getByRole('button', { name: 'common.delete' })
      );
      await waitFor(() => expect(remove).toHaveBeenCalledWith(7, 503));
    }
  );

  it('keeps recorded payment actions hidden for read-only members', async () => {
    serveSettlement({
      ...SETTLEMENT,
      settlements: [{ id: 504, from_user_id: 1, to_user_id: 2, amount: 25, currency: 'USD' }],
    });
    await renderTab(planner({ can: () => false }));
    const row = rowOf('costs.payment');
