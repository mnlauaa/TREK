...
vi.mock('../../../../src/mobile/screens/trip/sheets/MAccommodationSheet', () => ({
  default: selfRouted('stub-accommodation'),
}));
vi.mock('../../../../src/mobile/screens/trip/sheets/MTransportSheet', () => ({
  default: selfRouted('stub-transport'),
}));
vi.mock('../../../../src/mobile/screens/trip/sheets/MBrowseActionsSheet', () => ({
  default: selfRouted('stub-bract'),
}));
...
vi.mock('../../../../src/mobile/screens/trip/sheets/MReservationSheet', () => ({
  default: ({ onOpenExpense }: { onOpenExpense: (req: BookingExpenseRequest) => void }) => (
    <div data-testid="stub-reservation">
      ...
      <button type="button" onClick={() => onOpenExpense({})}>
        expense noop
      </button>
    </div>
  ),
...
vi.mock('../../../../src/components/Trips/TripFormModal', () => ({
  default: ({
    ...
    <div data-testid="stub-tripform" data-open={String(isOpen)} data-title={trip?.title ?? 'none'}>
      <button type="button" onClick={() => void onSave({ title: 'Japan 2027' })}>
        save trip
      </button>
      <button type="button" onClick={() => onCoverUpdate(1, '/uploads/covers/new.jpg')}>
        update cover
      </button>
      <button type="button" onClick={onClose}>
        close trip form
...
vi.mock('../../../../src/components/Trips/TripMembersModal', () => ({
  default: ({
    ...
    <div data-testid="stub-members" data-open={String(isOpen)} data-title={tripTitle ?? 'none'}>
      <button type="button" onClick={onMembersChanged}>
        members changed
      </button>
      <button type="button" onClick={onClose}>
        close members
...
vi.mock('../../../../src/components/Planner/TransitJourneyModal', () => ({
  default: ({
    ...
    <div data-testid="stub-transit" data-title={reservation.title} data-canedit={String(canEdit)}>
      <button type="button" onClick={() => void onSave({ title: 'Renamed' })}>
        save transit
      </button>
...
describe('MTripSheets', () => {
  ...
  it('FE-MOB-SHOST-015: falls back to the journey itself when it is not in the list', () => {
    const planner = buildPlanner({
      ...
      reservations: [],
      can: vi.fn(() => false) as TripPlanner['can'],
    });
