...
test('phone All Days remains selected on the map and restores a valid timeline day', async ({ page }) => {
  await page.goto('/dashboard');
  await dismissSystemNotices(page);
  const created = await page.request.post('/api/trips', {
    data: { title: `All Days ${Date.now()}`, start_date: '2099-06-01', end_date: '2099-06-05' },
  });
  expect(created.ok()).toBeTruthy();
  const body = await created.json();
  const tripId = body.trip?.id ?? body.id;
  await page.goto(`/trips/${tripId}`);
  await expect(activeChip(page)).toBeVisible({ timeout: 30_000 });
  const rail = activeChip(page).locator('xpath=..');
  await rail.locator('button').nth(2).click();
  const chosenDay = (await activeChip(page).textContent())!.trim();
  await page.getByRole('button', { name: 'Map view', exact: true }).click();
  const allDays = page.getByRole('button', { name: 'All Days', exact: true });
  await allDays.click();
  await expect(allDays).toHaveAttribute('aria-pressed', 'true');
  await expect(activeChip(page)).toHaveCount(0);
  await page.getByRole('button', { name: 'List view', exact: true }).click();
  await expect(activeChip(page)).toHaveText(chosenDay);
  await page.reload();
  await expect(activeChip(page)).toBeVisible({ timeout: 30_000 });
});
