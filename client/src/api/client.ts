...
export const tripsApi = {
  ...
  runNewMemberIdentityCheck: (id: number | string): Promise<NewMemberIdentityCheckResponse> =>
    apiClient.post(`/trips/${id}/new-member-identity-check`).then((r) => r.data),
  declineNewMemberIdentityCheck: (id: number | string) =>
    apiClient.post(`/trips/${id}/new-member-identity-check/decline`).then((r) => r.data),
  guestIdentityTransferCandidates: (id: number | string): Promise<GuestIdentityTransferCandidatesResponse> =>
...
export const assignmentsApi = {
  ...
  updateTransport: (
    ...
    apiClient
      .put(
        `/trips/${tripId}/assignments/${id}/transport`,
        (direction === 'incoming'
          ? { transport_mode: mode, direction }
          : { transport_mode: mode }) satisfies Partial<AssignmentTransportRequest>
      )
      .then((r) => r.data),
};
...
export const pluginsApi = {
  ...
  pluginRoute: (
    ...
    body: {
      tripId: number | string;
      dayId?: number | null;
      waypoints: Array<{ lat: number; lng: number; name?: string; placeId?: number }>;
    },
    opts: { signal?: AbortSignal } = {}
  ) =>
    apiClient
      .post(`/plugin-routes/${pluginId}/${profileId}`, body, { timeout: 25000, signal: opts.signal })
      .then((r) => r.data as { route: PluginRouteResult | null }),
...
export const journeyApi = {
  ...
  listTracks: (id: number): Promise<JourneyTracksResponse> =>
    apiClient.get(`/journeys/${id}/tracks`).then((r) => r.data),
  ...
  deleteBook: (id: number): Promise<void> => apiClient.delete(`/journeys/${id}/book`).then((r) => r.data),
...
export const memoriesApi = {
  ...
  albumPhotos: (provider: string, albumId: string, passphrase?: string, signal?: AbortSignal) =>
    apiClient
      .get(`/integrations/memories/${provider}/albums/${albumId}/photos`, {
...
export const mapsApi = {
  ...
  placeEnrichment: (
    ...
    apiClient
      .post('/maps/enrichment', body, { signal, timeout: 25000 })
  ...
  placePhotoCredit: (key: string) =>
    apiClient
      .get(`/maps/enrichment/credit/${encodeURIComponent(key)}`)
      .then((r) => r.data as { credit: string | null }),
  ...
  pois: (
    ...
    apiClient.get('/maps/pois', { params: { category, ...bbox, lang }, signal, timeout: 20000 }).then(
      (r) =>
        r.data as {
...
export const budgetApi = {
  ...
  reorderCategories: (tripId: number | string, orderedCategories: string[]) =>
    apiClient
      ...
      .then((r) => r.data),
  listExchangeRates: (tripId: number | string): Promise<{ rates: TripExchangeRate[] }> =>
    apiClient.get(`/trips/${tripId}/exchange-rates`).then((r) => r.data),
  resolveExchangeRate: (tripId: number | string, currency: string): Promise<ExchangeRateResolution> =>
    apiClient.get(`/trips/${tripId}/exchange-rates/resolve`, { params: { currency } }).then((r) => r.data),
  setExchangeRate: (tripId: number | string, currency: string, exchange_rate: number, note?: string | null) =>
    apiClient
      .put(`/trips/${tripId}/exchange-rates/${encodeURIComponent(currency)}`, { exchange_rate, note })
      .then((r) => r.data),
  deleteExchangeRate: (tripId: number | string, currency: string) =>
    apiClient.delete(`/trips/${tripId}/exchange-rates/${encodeURIComponent(currency)}`).then((r) => r.data),
  previewExchangeRate: (tripId: number | string, currency: string, exchange_rate: number, note?: string | null) =>
    apiClient
      .post(`/trips/${tripId}/exchange-rates/${encodeURIComponent(currency)}/preview`, { exchange_rate, note })
      .then((r) => r.data),
  applyExchangeRate: (
    tripId: number | string,
    currency: string,
    preview_id: string,
    selected: Array<{ type: 'expense' | 'settlement'; id: number }>
  ) =>
    apiClient
      .post(`/trips/${tripId}/exchange-rates/${encodeURIComponent(currency)}/apply`, { preview_id, selected })
      .then((r) => r.data),
};

export const exchangeRatesApi = {
  snapshot: (base = 'EUR'): Promise<ExchangeRateSnapshot> =>
    apiClient.get('/exchange-rates', { params: { base } }).then((r) => r.data),
};
...
export const reservationsApi = {
  ...
  setTravelers: (tripId: number | string, id: number, userIds: number[]) =>
    apiClient.put(`/trips/${tripId}/reservations/${id}/travelers`, { user_ids: userIds }).then((r) => r.data),
...
export const weatherApi = {
  ...
  get: (lat: number, lng: number, date: string, lang?: string, time?: string): Promise<WeatherResult> =>
    apiClient
      .get('/weather', { params: { lat, lng, date, lang, time } })
  ...
  getCurrent: (lat: number, lng: number, lang?: string): Promise<WeatherResult> =>
    apiClient
      ...
      .then((r) => parseInDev(weatherResultSchema, r.data, 'weather.getCurrent')),
...
export const configApi = {
  getPublicConfig: (): Promise<PublicConfig> => apiClient.get('/config').then((r) => r.data),
};
...
export const helpApi = {
  index: (): Promise<{ sections: HelpNavSection[] }> => apiClient.get('/help/index').then((r) => r.data),
...
export const notificationsApi = {
  ...
  testWebPushDevice: (id: number) => apiClient.post(`/notifications/web-push/devices/${id}/test`).then((r) => r.data),
};
