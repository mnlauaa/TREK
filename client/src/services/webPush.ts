...
const subscriptionJson = (subscription: PushSubscription): WebPushSubscription =>
  webPushSubscriptionSchema.parse(subscription.toJSON());
...
export async function setActivePushUser(userId: number | null): Promise<void> {
  try {
    ...
    /* private preview is used when state is unavailable */
  }
}
