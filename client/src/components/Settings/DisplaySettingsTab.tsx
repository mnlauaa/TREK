      ...
      <Section title={t('settings.general.languageRegion')} icon={Languages}>
        ...
        <div>
          ...
          <CurrencySelect
            ...
            onChange={async (v) => {
              try {
                await updateSetting('default_currency', v);
              } catch (e: unknown) {
                toast.error(e instanceof Error ? e.message : t('common.error'));
              }
            }}
            specialOptions={[{ value: '', label: t('settings.currencyTrip') }]}
            currentCurrency={settings.default_currency || ''}
          />
        ...
        <div>
          ...
          <p className="mb-2 text-xs text-content-faint">{t('settings.commonCurrencies.hint')}</p>
