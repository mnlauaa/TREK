...
export default function TripMembersModal({
  ...
  const [identityTransferOpen, setIdentityTransferOpen] = useState(false);
  const toast = useToast();
  ...
  return (
    <>
                  ...
                  {guests.map((g) => (
                    ...
                    >
                      ...
                      {!isCurrentOwner && renamingGuestId !== g.id && (
                        <button
                          type="button"
                          onClick={() => setIdentityTransferOpen(true)}
                          className="rounded-md border border-edge px-2 py-1 text-[0.65rem] text-content-muted"
                        >
      ...
      <GuestIdentityTransferModal
        tripId={tripId}
        open={identityTransferOpen}
        onClose={() => setIdentityTransferOpen(false)}
        onTransferred={() => loadMembers(true)}
      />
    </>
  );
}
