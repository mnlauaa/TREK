(function () {
  ...
  self.addEventListener('push', (event) => {
    event.waitUntil((async () => {
      ...
      const body = showFull && typeof payload.body === 'string'
        ? payload.body
        : 'Sign in to view your notification.';
      const path = showFull && typeof payload.path === 'string' && payload.path.startsWith('/')
        ? payload.path
        : '/';
      const unreadCount = payload && Number.isFinite(Number(payload.unreadCount))
  ...
  self.addEventListener('notificationclick', (event) => {
    ...
    event.waitUntil((async () => {
      ...
      const path = typeof rawPath === 'string' && rawPath.startsWith('/') && !rawPath.startsWith('//') ? rawPath : '/';
