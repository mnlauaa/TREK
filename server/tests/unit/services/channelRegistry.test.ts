...
describe('channelRegistry', () => {
  it('CHREG-001 — the four built-in external channels are registered; in-app is not', () => {
    expect(listChannels().map((c) => c.id)).toEqual(['email', 'webhook', 'ntfy', 'webpush']);
  ...
  it('CHREG-006 — only email declares the admin-scoped toggle bypass', () => {
    ...
    expect(getChannel('webpush')?.bypassesActiveToggleForAdminEvents).toBeUndefined();
  });
  ...
  it('CHREG-007 — webhook and ntfy deliver the admin-global copy; email does not', () => {
    ...
    expect(getChannel('webpush')?.supportsAdminGlobal).toBeUndefined();
  });
  ...
  it('CHREG-008 — built-ins carry every event except synology_session_cleared', () => {
    ...
    expect(getChannel('webpush')!.supportsEvent('trip_invite')).toBe(true);
    expect(getChannel('webpush')!.supportsEvent('version_available')).toBe(false);
    expect(getChannel('webpush')!.supportsEvent('synology_session_cleared')).toBe(false);
  });
