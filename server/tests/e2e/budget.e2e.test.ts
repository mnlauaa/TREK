...
describe('Budget e2e (real auth guard + temp SQLite, real budget SQL)', () => {
  ...
  async function build() {
    ...
    // Keep real frozen-rate writes and provenance; only the provider read is
    // stubbed for this all-EUR fixture so it never touches the network.
    vi.spyOn(moduleRef.get(ExchangeRatesService), 'getRates').mockResolvedValue(null);
