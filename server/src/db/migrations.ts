...
function runMigrations(db: Database.Database): void {
  ...
  const migrations: Migration[] = [
    ...
    // Migration 199: fork crosswalk for the schema-version collision at 176-180.
    // The previous fork used those five numeric slots for FX, Guest identity and
    // Web Push, while upstream v4 used them for these five unrelated features.
    // Existing fork databases therefore arrive here after running upstream
    // migrations 181-198 but without the artifacts below. Every operation is
    // idempotent, so clean upstream and fresh databases take the same path.
    () => {
      const hasColumn = (table: string, column: string): boolean =>
        (db.prepare(`SELECT 1 FROM pragma_table_info(?) WHERE name = ?`).get(table, column) as unknown) !== undefined;
      ...
      addColumn('vacay_entries', 'fraction REAL NOT NULL DEFAULT 1');
      db.exec(`
        CREATE TABLE IF NOT EXISTS vacay_shares (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          owner_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      ...
      `);
      addColumn('day_assignments', 'leg_transport_mode TEXT');
      addColumn('days', 'default_transport_mode TEXT');
      addColumn('vacay_plans', 'school_holidays_enabled INTEGER DEFAULT 0');
      addColumn('vacay_holiday_calendars', "type TEXT NOT NULL DEFAULT 'public_holiday'");
    },

    // Migration 200: final enhanced-FX schema. This deliberately creates the
    // post-preview shape directly: persisted quotes were never a released
    // contract, while trip defaults, durable snapshots and batch previews are.
    () => {
      db.exec(`
      ...
      for (const table of ['budget_items', 'budget_settlements']) {
        addColumn(table, "exchange_rate_source TEXT NOT NULL DEFAULT 'legacy'");
        addColumn(table, 'exchange_rate_source_version TEXT');
        addColumn(table, 'exchange_rate_effective_date TEXT');
        addColumn(table, 'exchange_rate_set_at TEXT');
        addColumn(table, 'exchange_rate_set_by_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL');
        addColumn(table, 'exchange_rate_note TEXT');
        addColumn(table, 'exchange_rate_reset_at TEXT');
    ...
    // Migration 201: one-time new-member identity-check state. A database that
    // already ran the fork migration keeps every completion/null value. Clean v4
    // installs mark existing account memberships complete so only future joins
    // receive the check.
    () => {
      const columns = db.prepare("SELECT name FROM pragma_table_info('trip_members')").all() as Array<{ name: string }>;
      const hadLegacy = columns.some((column) => column.name === 'guest_claim_prompted_at');
      const hadCompleted = columns.some((column) => column.name === 'new_member_identity_check_completed_at');
      if (!hadCompleted) {
    ...
    // Migration 202: Direct Web Push subscriptions. Rows are durable tombstones;
    // CREATE IF NOT EXISTS preserves existing subscriptions and VAPID identity.
    () => {
      db.exec(`
